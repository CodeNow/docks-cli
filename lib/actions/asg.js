...
/**
 * Determines the environment tag based on the environmnet option.
 * @return {String} The tag for environment.
 */
function getEnvTag() {
  var env = options.get('env');
  if (env === 'gamma') {
    return 'production-gamma';
  }
  return env;
}
    ...
    return aws.listAutoScalingGroups()
      ...
      .then(function (groups) {
        var table = new Table({
          ...
          colWidths: [36, 16, 24, 6, 6, 10, 12, 47]
        });
