  ...
  return aws.listAutoScalingGroups()
    .then(function (groups) {
      ...
      var orgGroup = groups.find(function (g) {
        ...
        var env = g.Tags.find(function (tag) { return tag.Key === 'env'; });
        var envToTest = 'production-' + options.get('env');
        return tag.Value === org && env.Value === envToTest;
      });
      if (exists(orgGroup)) {
        throw new CLIError(
  ...
  'scale-out': function () {
    return Promise
      ...
      .then(function (group) {
        ...
        var max = group.max;
        if (
          group.min + number > group.max ||
          group.desired + number > group.max
        ) {
          data.MaxSize = number;
        }
