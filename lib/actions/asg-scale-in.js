...
module.exports = Promise.method(function scaleInASGs () {
  const description = 'Remove instances from an auto-scaling group'
  ...
  return aws.listAutoScalingGroups(program)
    ...
    .then(function (group) {
      ...
      var number = parseInt(program.number)
      ...
      if (data.DesiredCapacity < 0) {
        throw new CLIError('The group cannot be scaled-in further.')
      }

      return rabbit.publishEvent('asg.update.requested', { githubId: org, data: data }, program)
    })
})
