...
module.exports = {
  ...
  list: function (options) {
    const connectSpinner = new Spinner(
      '%s :computer:  Connecting to '.emoji + 'Swarm Manager'.cyan + ' on ' + options.env.rainbow
    )
    connectSpinner.setSpinnerString('|/-\\')
    connectSpinner.start()
    
    const contextName = kubernetes.getClusterName(options)
