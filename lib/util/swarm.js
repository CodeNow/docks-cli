  ...
  getAllContainers: function (orgFilter) {
    ...
    return Promise
      ...
      .then(function () {
        connectSpinner.stop();
        queueSpinner.start();
        return execAsync('docker ps --no-trunc=true --format \'{{.Image}}|{{.Ports}}|{{.Label "instanceName"}}|{{.Label "ownerUsername"}}|{{.Label "com.docker.swarm.constraints"}}|{{.Names}}|{{.ID}}\' | grep registry')
      })
