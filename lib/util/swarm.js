          ...
          return Promise.resolve()
            .then(function () {
              ...
              return execAsync('docker -H tcp://localhost:53260 --tlsverify ' +
                ...
                'ps -a --no-trunc=true --format \'{{.Image}}|{{.Ports}}|{{.Label "instanceName"}}|{{.Label "ownerUsername"}}|{{.Label "com.docker.swarm.constraints"}}|{{.Names}}|{{.ID}}|{{.Status}}{{.RunningFor}}\' | grep localhost | grep -v seconds | grep -v "1 Minutes"', {maxBuffer: 1024 * 500})
            })
