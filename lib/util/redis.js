...
module.exports = {
  ...
  spawnTunnel: function () {
    ...
    if (options.get('env') === 'production') {
      hostname = 'alpha-redis';
    }
    else if (options.get('env') === 'gamma') {
   ...
   */
  removeDockFromWeave: function (ip, org) {
    ...
    return this.getClient()
      .then(function (client) {
        actionSpinner.start();
        if (options.has('dry')) { return Promise.delay(250); }
