...
module.exports = {
  ...
  askList: function (prompt, list, defaultAnswer) {
    var numChoices = list.length;
    list = list.map(function (l, i) { return '[' + (i + 1) + ']: ' + l; });
    list.unshift(prompt);
    list.push('');
    list.push(':interrobang:  '.emoji +
      'Please make a selection [1-' + numChoices + ']')
    var question = list.join('\n');
    return this.ask(question, -1)
      .then(function (answer) {
        ...
        if (answer < 1 || answer > numChoices) {
          throw new AbortedError('Selection was invalid [out-of-bounds].');
        }
        return answer;
      })
  },
